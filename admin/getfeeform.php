...
$sid = (isset($_POST['student']))?filter_var($_POST['student'],FILTER_SANITIZE_STRING):'';
 ...
 $sql = "select s.registration_id,s.name,s.balance,s.fees,s.contact_no,b.program,s.doj from users as s,program as b where b.program=s.program and  s.delete_status in ('0','2') and s.registration_id='".$sid."'";
 $q = $conn->prepare($sql);
 $q->execute();
...
$sid = (isset($_POST['student']))?filter_var($_POST['student'],FILTER_SANITIZE_STRING):'';
$sql = "select paid,submitdate,transaction_remark,billno from fees_transaction  where stdid='".$sid."'";
$fq = $conn->prepare($sql);
$fq->execute();
#$fq = $conn->query($sql);
if($fq->rowCount() > 0)
{
 ...
 $sql = "select s.registration_id,s.name,s.balance,s.fees,s.contact_no,b.program,s.doj,f.billno from users as s,program as b,fees_transaction as f where b.program=s.program and s.registration_id=f.stdid and s.registration_id='".$sid."' order by f.billno desc";
 $sq = $conn->prepare($sql);
 $sq->execute();
#$sq = $conn->query($sql);
...
$sid = (isset($_POST['student']))?filter_var($_POST['student'],FILTER_SANITIZE_STRING):'';
$sql = "select paid,submitdate,transaction_remark,billno from fees_transaction  where stdid='".$sid."' and billno = ( select max(billno) from fees_transaction  where stdid='".$sid."')" ;
$fq = $conn->prepare($sql);
$fq->execute();
#$fq = $conn->query($sql);
if($fq->rowCount() > 0)
