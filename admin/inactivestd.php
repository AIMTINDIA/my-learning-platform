...
if(isset($_GET['action']) && $_GET['action']=="approve"){
     $sqldelete = $conn->prepare("SELECT * FROM users WHERE registration_id='".$_GET['registration_id']."'");
 $sqldelete->execute();
 $rowsdelete = $sqldelete->fetch(PDO::FETCH_ASSOC);
 $emailid = $rowsdelete['email'];
 $d1=$conn->prepare("update otp set user_type='student' where email='$emailid'");
 $d1->execute();
                            ...
                            <div class="table-sorting table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="tSortable22">
									...
									echo '<tr '.(($r['balance']>0)?'class="primary"':'').'>
											...
											<a href="inactivestd.php?action=approve&registration_id='.$r['registration_id'].'" class="btn-success btn-xs" style="border-radius:60px;"><span class="glyphicon glyphicon-ok"></span></a>
											
											<a onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" href="inactivestd.php?action=delete&registration_id='.$r['registration_id'].'" class="btn-danger btn-xs" style="border-radius:60px;"><span class="glyphicon glyphicon-trash"></span></a> </td>
