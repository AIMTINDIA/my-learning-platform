  ...
  <div class="form-group">
    ...
    <select  class="form-control" id="program" name="program" >
		<option value="" >Select Program</option>
                                    <?php
									$sql = $conn->prepare("select * from `program` where delete_status='0' order by program.program asc");
                                    $sql->execute();
									
									while($r = $sql->fetch(PDO::FETCH_ASSOC))
									{
									echo '<option value="'.$r['id'].'"  '.(($program==$r['id'])?'selected="selected"':'').'>'.$r['program'].'</option>';
									}
									?>
	</select>
  </div>
...
display:none;
  ...
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog modal-lg" >
      <div class="modal-content" id="printcontent">
        <div class="modal-header" style="text-align:center;">
          ...
          <pre style="text-align:left;">                                                                                                     Receipt                                                                                       Mob: +91-8439515173                                                                                                                                       </pre>
          <img src="logo1.png" style="position:absolute;top:87px;left:45px;width:8rem;height:8rem;border-radius:50%;">
          <h4 class="modal-title" style="font-weight:bold;">Blackhawk Institue of Research & Development (BIRD)</h4>
		  <h5>A unit of Telecall Technology Pvt. Ltd.</h5>
		  <h5>Add. Plot 6 near tata motors showroom sikandrabad Bulandshahr (U.P.) </h5>
        </div>
        ...
        <div class="modal-footer">
		  ...
		  <button type="button" class="btn-danger" style="border-radius:0% ;padding: 6px 12px " id='Print'  onclick='handlePrint()' >Print</button> 
        </div>
      </div>
    </div>
  </div>
   ...
   <script type="text/javascript">
         const handlePrint = () => {
            var actContents = document.getElementById("printcontent").innerHTML;
