...
$id = unique_id();
 ...
 $image = $_FILES['image']['name'];
  ...
  $select_user = $conn->prepare("SELECT * FROM `users` WHERE email = ?");
   $select_user->execute([$emailid]);
   
   if( strlen($emailid)<5 || $select_user->rowCount() > 0  ){
      $errormsg = "<div class='alert alert-danger'> <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Email already taken!!</div>";
	}else{
  ...
  $userType = "student";
   ...
   if($advancefees >= 9000){
         ...
         $insert_otp->execute([$emailid,$userType,$registration_id]);
   }
