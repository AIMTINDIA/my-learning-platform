...
$sql = "select fees,balance,email  from users where registration_id = '$sid'";
$sq = $conn->query($sql);
...
$select_otp = $conn->prepare("SELECT * FROM `otp` WHERE email = ?");
   $select_otp->execute([$email]);
 if($totalpaid >= 9000 && $select_otp->rowCount() == 0){
		 $insert_otp = $conn->prepare("INSERT INTO `otp`(email,user_type,stdid) VALUES(?,?,?)");
         $insert_otp->execute([$email,$userType,$sid]);
   }

$sql = "update users set balance='$tbalance' where registration_id = '$sid'";
$conn->query($sql);
...
$(document).ready( function() {
    ...
    $("#doj").focus(function () {
		...
		function mydatatable()
        {
              ...
              $("#subjectresult").html('<table class="table table-striped table-bordered table-hover" id="tSortable22"><thead><tr><th>Name/Contact</th><th>Fees</th><th>Balance</th><th>Program</th><th>DOJ</th><th>Action</th></tr></thead><tbody></tbody></table>');
